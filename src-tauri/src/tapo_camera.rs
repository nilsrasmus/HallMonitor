...
impl TapoCamera {
    ...
    pub async fn get_privacy_mode(&self) -> Result<PrivacyMode, TapoError> {
        ...
        let enabled = resp["result"]["responses"][0]["result"]["lens_mask"]["lens_mask_info"]
            ...
            .as_str()
            .or_else(|| {
                resp["result"]["lens_mask"]["lens_mask_info"]["enabled"]
                    .as_str()
            })
            .unwrap_or("off");
    ...
    pub async fn move_motor(&self, pan: i32, tilt: i32) -> Result<(), TapoError> {
        if pan == 0 && tilt == 0 {
            return Ok(());
        }
        ...
        let cmd = serde_json::json!({
            ...
            "motor": {
                "move": {
                    "x_coord": pan.to_string(),
                    "y_coord": tilt.to_string(),
                }
            }
        });

        let resp = self.send_command(cmd).await?;
        let code = resp["error_code"].as_i64().unwrap_or(0);
        if code != 0 {
            return Err(TapoError::Device {
                code,
                message: format!("motor move failed: {resp:?}"),
            });
        }
        Ok(())
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn native_tls_reaches_camera() {
        let ip = std::env::var("TAPO_IP").unwrap_or_else(|_| "192.168.1.63".into());
        let camera = TapoCamera::new(&ip, "user", "pass").expect("client build");
        let probe = serde_json::json!({
            "method": "login",
            "params": {
                "encrypt_type": "3",
        ...
        let resp = camera
            .client
            .post(format!("https://{ip}/"))
            .headers(TapoCamera::default_headers())
            .json(&probe)
            .send()
            .await
            .expect("HTTPS request should reach camera");
